let run ~name ~email ~password ~display_name ~bio ~avatar_url ~links =
  ...
  let* hashed_password = Domains.Values.Password.from password in
  let* display_name = User.DisplayName.from display_name in
  let* bio = User.Bio.from bio in
  let* avatar_url = User.AvatarUrl.from avatar_url in
  let* links = User.Links.from links in
  Db.Users.create ~name ~email ~display_name ~bio ~avatar_url ~hashed_password ~links
  >>| Domains.Objects.User.id
;;
