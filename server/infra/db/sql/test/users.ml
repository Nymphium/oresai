...
let tests =
  ...
  , [ ( test_case "insert" `Quick @@ fun () ->
    ...
    ; ( test_case "no verify" `Quick @@ fun () ->
        let res = Sql.Users.verify 0L >>| Option.map ~f:(Fn.compose Id.to_ fst) in
        Alcotest.check'
          (result (option int64) Errors.t)
          ~msg:"equal"
          ~expected:(Ok None)
          ~actual:res )
    ; ( test_case "verify" `Quick @@ fun () ->
        ...
        let user =
          ...
          (let hashed_password = Domains.Values.Password.unsafe_from plain in
           Sql.Users.create
             ...
             ())
          |> function
          | Error e -> Alcotest.fail @@ Sql.Errors.show e
          | Ok user -> user
        in
        let res =
          Sql.Users.verify (id user)
        ...
        Alcotest.check'
          ...
          ~actual:res )
    ; ( test_case "find_by_id" `Quick @@ fun () ->
        let fx =
          Model.fixture
            ~name:"john smith"
        ...
        let res =
          ...
          | Ok user ->
            ...
            user
        in
        let res' = Sql.Users.find_by_id (id res) in
        Alcotest.check'
          (result (option Model.t) Errors.t)
          ~msg:"equal"
          ~expected:(Ok (Some fx))
          ~actual:Model.(res' >>| Option.map ~f:omit_ts >>| Option.map ~f:omit_id) )
    ; ( test_case "find_by_email" `Quick @@ fun () ->
        let fx =
          Model.fixture
            ~name:"john smith"
            ~email:"hoge35@email.invalid"
            ~display_name:"jsmth"
            ~bio:"hello"
            ~avatar_url:"https://example.invalid"
            ()
        ...
        let res =
          ...
          | Ok user ->
            ...
            user
        in
        let res' = Sql.Users.find_by_email (email res) in
        Alcotest.check'
          (result (option Model.t) Errors.t)
          ~msg:"equal"
          ~expected:(Ok (Some fx))
          ~actual:Model.(res' >>| Option.map ~f:omit_ts >>| Option.map ~f:omit_id) )
    ; ( test_case "links" `Quick @@ fun () ->
        let fx =
          Model.fixture
            ~name:"john smith"
    ...
    ; ( test_case "empty update" `Quick @@ fun () ->
        let fx =
          Model.fixture
            ~name:"john smith"
        ...
        let user =
          ...
          | Ok t ->
            ...
            t
        in
        let user' = Sql.Users.update user.id () in
        Alcotest.check'
          (result Model.t Errors.t)
          ~msg:"updated"
          ~expected:(Ok fx)
          ~actual:Model.(user' >>| omit_id >>| omit_ts) )
    ; ( test_case "update" `Quick @@ fun () ->
        let fx =
          Model.fixture
            ~name:"john smith"
        ...
        let user' = Sql.Users.update user.id ~bio () in
        Alcotest.check'
          (result Model.t Errors.t)
          ~msg:"updated"
