...
open Alcotest
open Core
open Domains.Objects.Article
module Model = Testing.Article
  ...
  , [ ( test_case "insert" `Quick @@ fun () ->
        let user_fx =
          Testing.User.fixture
            ~name:"john smith"
            ~email:"hoge+article@email.invalid"
            ~display_name:"jsmth"
            ~bio:"hello"
            ()
        in
        ...
        in
        let res =
          let { title; content; user_id; tags; state; _ } = fx in
          Sql.Articles.create ~title ~content ~user_id ~tags ~state
        in
        Alcotest.check'
