...
module Model = struct
  ...
  let t =
    let base_t =
      Caqti_type.(Std.t7 int64 string string string string (option string) ptime)
    in
    let encode fwd =
    ...
    let decode (id, name, email, display_name, bio, avatar_url, created_at) =
      ...
      let avatar_url = Option.map ~f:Domains.Values.Url.unsafe_from avatar_url in
...
let create ~name ~email ~display_name ~bio ~hashed_password:password ?avatar_url ~links ()
  ...
  let* id, created_at =
    [%rapper
      ...
      ~bio:(Fwd.Bio.to_ bio)
      ~avatar_url:(Option.map ~f:Fwd.AvatarUrl.to_ avatar_url)
      ~password
      db
  in
